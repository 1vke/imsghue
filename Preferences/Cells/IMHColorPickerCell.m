#import "IMHColorPickerCell.h"

@implementation IMHColorPickerCell {
...
- (void)openColorPicker {
    ...
    if (@available(iOS 14.0, *)) {
        ...
        if ([preferences objectForKey:self.specifier.properties[@"key"]]) {
            colourPickerVC.selectedColor = [self colorWithHexString:[preferences objectForKey:self.specifier.properties[@"key"]]];
        } else {
...
- (void)updatePreview {
    ...
    if ([preferences objectForKey:self.specifier.properties[@"key"]]) {
        self.colorPreview.backgroundColor = [self colorWithHexString:[preferences objectForKey:self.specifier.properties[@"key"]]];
    } else {
...
- (void)colorPickerViewControllerDidSelectColor:(UIColorPickerViewController *)viewController API_AVAILABLE(ios(14.0)){
    ...
    [preferences setObject:[self hexStringFromColor:selectedColour] forKey:self.specifier.properties[@"key"]];
    [self updatePreview];
}
